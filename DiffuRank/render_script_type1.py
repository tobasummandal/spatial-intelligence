...
def _get_socket(socket_map, name_or_names):
    if isinstance(name_or_names, str):
        name_or_names = [name_or_names]
    for name in name_or_names:
        if name in socket_map:
            return socket_map[name], name
    return None, None
...
def setup_material_extraction_shader_for_material(mat, capturing_material_alpha: bool):
    ...
    for key, names in required.items():
        socket, actual_name = _get_socket(socket_map, names)
        if socket is None:
            # Missing emission sockets happens with Principled v2 when emission not exposed; skip extraction.
            return None
        sockets[key] = socket

    old_base_color = get_socket_value(mat.node_tree, sockets["base_color"])
    old_alpha = get_socket_value(mat.node_tree, sockets["alpha"])
    old_emission = get_socket_value(mat.node_tree, sockets["emission"])
    old_emission_strength = get_socket_value(mat.node_tree, sockets["emission_strength"])
    old_specular = get_socket_value(mat.node_tree, sockets["specular"])
    ...
    # to make the pixels.
    clear_socket_input(mat.node_tree, sockets["base_color"])
    sockets["base_color"].default_value = [0, 0, 0, 1]
    clear_socket_input(mat.node_tree, sockets["alpha"])
    sockets["alpha"].default_value = 1
    clear_socket_input(mat.node_tree, sockets["specular"])
    sockets["specular"].default_value = 0.0

    old_blend_method = mat.blend_method
    mat.blend_method = "OPAQUE"

    if capturing_material_alpha:
        set_socket_value(mat.node_tree, sockets["emission"], old_alpha)
    else:
        set_socket_value(mat.node_tree, sockets["emission"], old_base_color)
    clear_socket_input(mat.node_tree, sockets["emission_strength"])
    sockets["emission_strength"].default_value = 1.0

    def undo_fn():
        mat.blend_method = old_blend_method
        set_socket_value(mat.node_tree, sockets["base_color"], old_base_color)
        set_socket_value(mat.node_tree, sockets["alpha"], old_alpha)
        set_socket_value(mat.node_tree, sockets["emission"], old_emission)
        set_socket_value(mat.node_tree, sockets["emission_strength"], old_emission_strength)
        set_socket_value(mat.node_tree, sockets["specular"], old_specular)
...
def _set_available_denoiser():
    """Choose a denoiser supported by the current Blender build/hardware."""
    enum_items = bpy.context.scene.cycles.bl_rna.properties["denoiser"].enum_items.keys()
    if "OPTIX" in enum_items:
        bpy.context.scene.cycles.denoiser = "OPTIX"
        bpy.context.scene.cycles.use_denoising = True
    elif "OPENIMAGEDENOISE" in enum_items:
